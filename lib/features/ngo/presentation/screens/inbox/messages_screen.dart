...
class _MessagesScreenState extends State<MessagesScreen> {
  ...
  final List<Map<String, String>> _donors = [
    {"name": "Allah Malik", "last": "Hello! Allah Malik here.", "time": "Now"},
    {"name": "Hotel Jafson", "last": "Hello! Hotel Jafson here.", "time": "Yesterday"},
    {"name": "Taj Hotel", "last": "Hello! Taj Hotel here.", "time": "2 days ago"},
  ];
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  leading: CircleAvatar(
                    ...
                    child: Text(donor["name"]![0], style: const TextStyle(color: Colors.black)),
                  ),
                  title: Text(donor["name"]!, style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(donor["last"]!, maxLines: 1, overflow: TextOverflow.ellipsis),
                  trailing: Text(donor["time"]!, style: const TextStyle(fontSize: 12)),
                  onTap: () async {
                    await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => ChatScreen(donorName: donor["name"]!),
                      ),
                    );

                    final msgs = ChatStore.getMessages(donor["name"]!);
                    if (msgs.isNotEmpty) {
                      final lastMsg = msgs.last;
                      setState(() {
                        donor["last"] = lastMsg.text;
                        donor["time"] = _formatTime(lastMsg.time);
                      });
                    }
                  },
                );
